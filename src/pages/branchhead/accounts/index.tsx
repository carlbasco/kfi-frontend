...
const Accounts = () => {
  const { data } = useSWR('/api/branchhead/accounts')
...
const NewDialog = ({ open, handleDialog }: DialogProps) => {
  const { data: roleApi } = useSWR('/api/option/branch/roles', {
    refreshInterval: 0,
  })
  ...
  const {
    ...
    defaultValues: BranchAccountUpdateDefaultValues,
    resolver: yupResolver(BranchAccountUpdateFormYup),
  })

  const onSubmit: SubmitHandler<BranchAccountUpdateForm> = async (values) => {
    try {
      setFetchLoading(true)
      const res = await ApiAuth.post(`/api/branchhead/account`, values)
...
const EditDialog = ({ open, handleDialog, id }: EditDialogProps) => {
  const { data } = useSWR(open ? `/api/branchhead/account/${id}` : null, {
    refreshInterval: 0,
  })
  const [fetchLoading, setFetchLoading] = useState(false)
...
const DeleteDialog = (props: EditDialogProps) => {
  ...
  const onSubmit = async () => {
    try {
      setFetchLoading(true)
      const res = await ApiAuth.delete(`/api/branchhead/account/${props.id}`)
      const data = await res.data
      mutate(`/api/accounts`)
      Snackbar.success(data.message)
      setFetchLoading(false)
