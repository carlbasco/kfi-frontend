...
import { ImageViewer, Loading, PhoneNumberFormat, Upload } from '@components'
...
import {
    AdminCaseFormUpdateYup, AdminCaseUpdateDefaultValues, AdminCaseUpdateForm
} from '@validation'
...
const EditCase = () => {
  ...
  const { data, error } = useSWR(id ? `/api/admin/case/${id}/edit` : null)
  ...
  const {
    ...
    defaultValues: AdminCaseUpdateDefaultValues,
    resolver: yupResolver(AdminCaseFormUpdateYup),
  })
  ...
  const onSubmit: SubmitHandler<AdminCaseUpdateForm> = async (values) => {
    const fd = new FormData()
    files.forEach((file: any) => fd.append('files', file))
    fd.append('data', JSON.stringify(values))
    try {
      setFetchLoading(true)
      const res = await ApiAuth.put(`/api/admin/case/${id}`, fd)
      const data = await res.data
  ...
  useEffect(() => {
    if (data && option) {
      ...
      setValue('startDate', data.startDate ? data.startDate : '')
      setValue('endDate', data.endDate ? data.endDate : '')
