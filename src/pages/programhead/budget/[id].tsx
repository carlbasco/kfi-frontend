...
import { ArrowForwardIosSharp, ThumbDown, ThumbUp } from '@mui/icons-material'
...
const Budget = () => {
  ...
  const handleConfirmationBudget = () => {
                  ...
                  {data?.status === 'pending' && (
                    ...
                    >
                      <LoadingButton
                        color="secondary"
                        variant="contained"
                        onClick={handleConfirmationBudget}
                      >
                        Mark as completed
                      </LoadingButton>
                      <ConfirmationBudgetDialog
                        id={data?.id}
                        open={confirmationDialog}
                        handleDialog={handleConfirmationBudget}
                      />
                    </Box>
                  )}
                </>
...
const ConfirmationBudgetDialog = (props: DialogProps) => {
  const router = useRouter()
  const id = router.query.id
  const [fetchLoading, setFetchLoading] = useState(false)
  const handleCloseDialog = () => {
  ...
  const onSubmit = async () => {
    try {
      setFetchLoading(true)
      const res = await ApiAuth.post(`/api/head/budget/${props.id}/completed`)
      const data = await res.data
      mutate(`/api/case/${id}/budget`)
      Snackbar.success(data.message)
      setFetchLoading(false)
  ...
  return (
    <Dialog fullWidth maxWidth="xs" open={props.open}>
      ...
      <DialogContent>
        <DialogContentText>
          Are you sure you want to mark this budget disbursment as completed?
        </DialogContentText>
        <DialogActions>
